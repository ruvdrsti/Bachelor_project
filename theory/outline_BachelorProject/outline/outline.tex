...
This wave function $\Phi_0$ is a better approximation to the real wave function $\Gamma$ than $\Psi_0$. When we try to get the energy expectation value of this function, we can write it in matrix
...
between single excited states and the ground state wave function when taking the expectation value over the Hamiltonian. This makes sense, because if there was, the ground state
...
UHF and CUHF were generated using the GQCP\cite{GQCP} library.
\paragraph*{}
...
unconverged data might be interpreted wrong. We can see that across the plotted data, the spin contamination remains zero. The curve is also smoother than the UHF curve. Where 
...
said in the theory section. Since we used the theorem, it cannot be invalid now. The code needs to be updated. \\
Now we need to compare the data to ROHF theory.
...
We expect a different result from CUHF CIS than for ROHF CIS. Since when we look at the essence, at its core CUHF method is still an unrestricted method. It has two distinct Fock
operators and thus will have two sets of orbitals. The excitation scheme will look more like a UHF excitation scheme, because there will be more differences between the energy 
levels than in ROHF, where there are more doubly occupied orbitals than singly occupied orbitals\cite{Scuseria2011}. 
...
next step is full CI using the CUHF wave function. When a true correlation energy is present, we could do a stretch of \ce{H3} in order to see whether we can get the symmetry and
