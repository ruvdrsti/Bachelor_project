...
All the properties of a molecule are linked to the wave function $\Psi$, but it does not give us any information about how to extract them from it. This information must be gathered
using operators. The Schrödinger equation allows us
to calculate this wave functions energy $E$ using an operator know as the Hamiltonian. In its full form it can be seen in the equation below\cite{Szabo1996}.
...
directly linked to quantum numbers $S$ and $M_S$\cite{Acke2019}. These are linked to the symmetry in a molecule in the following way. We can externally define these quantum numbers,
and we can immediately interpret them. For example the spin state with $S=1$ can have $M_S \in \{-1,0,1\}$. This is called a triplet state. Every eigenfunction of the Hamiltonian can
...
namely that $\delta E$ should remain zero, we cannot state that the resulting approximative wave function is still an eigenfunction of $\hat{S}^2$.
This leads us to Löwdin's symmetry dilemma \cite{Lowdin1963}. When we enforce the symmetry and the energy constraint at the same time, for example by forcing the electrons into the
same orbitals, the minimum energy we reach is not the global minimum. However, when we drop the symmetry constraint and only enforce the energy, we will reach a lower energy,
but will lose all the information with regard to the symmetry of the system.
\paragraph*{}
...
\begin{equation}\label{eq:hamexp}
  \rangle \hat{H} \rangle = \begin{bmatrix}
    \langle \Psi_0 | \hat{H} | \Psi_0 \rangle         & \langle \Psi_0 | \hat{H} | \Psi_r^a \langle         & \cdots & \langle \Psi_0 | \hat{H} | \Psi_{rs}^{ab} \rangle  & \cdots \\
    \langle \Psi_r^a | \hat{H} | \Psi_0 \rangle       & \langle \Psi_r^a | \hat{H} | \Psi_r^a \rangle       & \cdots & \langle \Psi_r^a |\hat{H} | \Psi_{rs}^{ab} \rangle & \cdots \\
    \vdots                                            & \vdots                                              & \vdots & \vdots                                             & \vdots \\
    \langle \Psi_{rs}^{ab} | \hat{H} | \Psi_0 \rangle & \langle \Psi_{rs}^{ab} | \hat{H} | \Psi_r^a \rangle & \cdots & \langle \Psi_{rs}^{ab} | \hat{H} | \Psi_{rs}^{ab}  & \cdots \\
...
\eqref{eq:hamexp} can be simplified quite a bit. Then we already know that the expectation value of the Hartree-Fock wave function is the energy we can get from our normal
...
\paragraph*{}
For CIS \ce{H3} will be used as a model system. Firstly because it is a simple system containing few electrons. This means that we do not need to track a lot of possible excitations, so
...
\label{subsec:cis}
We will compare the CIS results for \ce{H3} as generated by our implementation and the one used by Psi4 and GQCP. GQCP will generate the data for CUHF and UHF and Psi4 will generate
the data for ROHF. In \ce{H3} we can consider nine excitations in total, so when we include the ground state, we should have ten states in total. We can take a closer look at
...
the GQCP library as 0.5, which means that it still is a spin eigenstate. The state was however not a stable one. We can learn from this that for open shell species that have spin
deficiencies CIS is not the method of choice if we want to include the triplet excitations. When the triplet excitations are left out of the calculation, we do find the expected
result, in which the ground state is the lowest energy state. These results were displayed in table \ref{tab:excits_no_triplets}\\
...
It is logical that in this picture it does not make a difference what spin the electron has towards the energy. So in fact we expect to see a degeneracy at the ground state energy
level. Since this does involve an alpha to beta excitation we do not expect to see it when leaving those out. The other degeneracy was seen at the energy level of $-2.176620$ 
...
Based on this picture we expect the same excitation if the alpha electron is excited. Now we need to verify that this is in fact the case. In a UHF method, we expect the alpha and
...
\begin{figure}[h]
  ...
  \caption{Orbital 1 for both alpha and beta electrons}
  \label{fig:1_cuhf}
\end{figure}
...
point group. The reason it does this is that it does not compute excitations between different irreducible representations. Given that all the orbitals for ROHF shown belong to 
...
Interestingly enough this orbital is the same as the one orbital of CUHF we have not seen, the second highest beta orbital. Indeed, when we take a look at figure 
...
noticed that both CUHF and UHF feature excited states that are lower in energy than the ground state, when allowing for alpha-to-beta and beta-to-alpha excitations. We then 
concluded that CIS is not the ideal method for describing such systems if one wants to account for such excitations. We can use CIS if we do not consider such excitations. 
